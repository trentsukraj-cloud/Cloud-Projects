...
In the top left of the screen, verify the New VPC Experience is toggled on. If it is not, toggle it on now. 
...
Change Public subnet CIDR block in us-west-2a to 10.0.0.0/24
Change Private subnet CIDR block in us-west-2a to 10.0.1.0/24
...
Public subnet name: lab-subnet-public1-us-west-2a
Private subnet name: lab-subnet-private1-us-west-2a
Route tables
...
The lab-subnet-public1-us-west-2a public subnet has a CIDR of 10.0.0.0/24, which means that it contains all IP addresses starting with 10.0.0.x. The fact the route table associated with this public subnet routes 0.0.0.0/0 network traffic to the internet gateway is what makes it a public subnet.
...
The  lab-subnet-private1-us-west-2a private subnet has a CIDR of 10.0.1.0/24, which means that it contains all IP addresses starting with 10.0.1.x.

Note: You may also notice Default VPC and Subnets. Those are created when the AWS Account is provisioned for the lab. It is not used in this lab and you can ignore it.
...
Availability Zone: Select the second Availability Zone (for example, us-west-2b)
...
Select  the lab-rtb-private1-us-west-2a route table.
...
Leave lab-subnet-private1-us-west-2a selected, but also select  lab-subnet-private2.
...
Leave lab-subnet-public1-us-west-2a selected, but also select  lab-subnet-public2.
...
# Download Lab files
wget https://aws-tc-largeobjects.s3.us-west-2.amazonaws.com/CUR-TF-100-PRYODA-1-37918/3-lab-vpc-web-server/s3/lab-app.zip
