...
NOTE: THIS WAS A GUIDED LAB DURING MY AWS RE/START SOLUTIONS ARCHITECT ASSOCIATE PROGRAMME, THE LAB WAS GRADED TOWARDS MY FINAL SCORE
...
Open the AWS Cloud9 service page and for DEVCafeServer, choose Open.

The AWS Cloud9 IDE that runs on an EC2 instance should now display.
...
In the AWS Management Console, select the  Services menu, and then select Cloud9 under Developer Tools.

On the left, expand the menu by choosing the    (menu icon), then choose Environments

Under Environments, from the dropdown, choose Shared with me, and verify that the DEVCafeServer environment is available.
...
Back in the browser where you are logged in as the voclabs user (Sofía), create an IAM group named DBAdministrators, with the following permissions:
...
As Olivia, update the dbUser value in the Systems Manager Parameter Store.
